...
@section('content')
    <h1 class="font-semibold text-2xl ml-6 mt-5">Surat Keluar</h1>
    @if (session('success'))
    ...
    <div class="flex justify-between items-end w-[81%]">
        <div id="search" class="mt-5 ml-60 w-[50%]">
            ...
            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                ...
                <div id="search-container" class="relative w-full">
                    <form action="{{route('searchOutgoing')}}" method="POST">
                        @csrf
        ...
        <a href="{{route('viewSuratKeluar')}}" class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg mr-40 text-sm px-5 py-2.5 border dark:bg-cyan-600 dark:hover:bg-cyan-700 dark:focus:ring-cyan-800">Lihat Semua</a>
        <a href="{{route('viewAddOutgoingMessage')}}" class="focus:outline-none text-white w-26 h-13 border bg-green-600 hover:bg-green-700 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"><svg height="17px" width="17px" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 309.059 309.059" xml:space="preserve" fill="#000000"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path style="fill:#ffffff;" d="M280.71,126.181h-97.822V28.338C182.889,12.711,170.172,0,154.529,0S126.17,12.711,126.17,28.338 v97.843H28.359C12.722,126.181,0,138.903,0,154.529c0,15.621,12.717,28.338,28.359,28.338h97.811v97.843 c0,15.632,12.711,28.348,28.359,28.348c15.643,0,28.359-12.717,28.359-28.348v-97.843h97.822 c15.632,0,28.348-12.717,28.348-28.338C309.059,138.903,296.342,126.181,280.71,126.181z"></path> </g> </g> </g></svg></a>
    </div>

    <div class="block w-[80%] mt-5 ml-5 p-6 bg-gray-300 border border-black rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 overflow-y-auto">
        @if ($suratKeluarCount->total() > 0)
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-black dark:text-gray-400">
                    <thead class="text-sm border-b border-black text-black uppercase bg-transparent dark:bg-gray-700 dark:text-gray-400">
                        <tr>
                    ...
                    <tbody>
                        @foreach ($suratKeluarCount as $item)
                            <tr class="bg-transparent border-black border-b dark:bg-gray-800 dark:border-gray-700">
                                <td class="px-6 py-4">
                                    {{$item->no_surat}}
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <a href="{{route('viewOutgoingMessage', ['id' => $item->id])}}" class="focus:outline-none text-white border border-black bg-cyan-800 hover:bg-cyan-900 font-medium rounded-lg text-sm px-4 py-2 me-2 dark:focus:ring-yellow-900">Lihat</a>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-row gap-1">
                                        <form action="{{route('viewEditOutgoingMessage', ['id' => $item->id])}}" method="GET">
        ...
        <div class="flex flex-row justify-between items-center mt-5">
            ...
            <span class="text-sm text-gray-700 dark:text-gray-400">
                @if ($suratKeluarCount->total() > 0)
                    Showing <span class="font-semibold text-gray-900 dark:text-white">{{ ($suratKeluarCount->currentPage() - 1) * $suratKeluarCount->perPage() + 1 }}</span> to <span class="font-semibold text-gray-900 dark:text-white">{{ $suratKeluarCount->lastItem() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $suratKeluarCount->total() }}</span> Entries
                @else
            ...
            <div class="inline-flex gap-2 mt-2 xs:mt-0">
                @if ($suratKeluarCount->currentPage() > 1)
                    <a href="{{ $suratKeluarCount->previousPageUrl() }}" class="flex items-center justify-center px-4 h-10 text-base border font-medium text-white bg-gray-800 rounded hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                ...
                @endif
                @if ($suratKeluarCount->hasMorePages())
                    <a href="{{ $suratKeluarCount->nextPageUrl() }}" class="flex items-center justify-center px-4 h-10 text-base font-medium border text-white bg-gray-800 rounded hover:bg-gray-900 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                        Berikutnya
            ...
            <div class="relative p-4 w-full max-w-md max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-4 md:p-5 text-center">
                        ...
                        <form action="{{route('deleteOutgoingMessage', ['id' => $item->id])}}" method="POST">
                            @csrf
                            @method('DELETE')
...
@section('container')
<div class="flex-grow @if ($suratKeluarCount->total() > 3)
    pb-44
@else
   pb-12
@endif">
