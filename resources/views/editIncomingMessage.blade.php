...
@section('content')
    <div class="flex flex-col justify-center items-center w-[81%] ml-2">
        ...
        @endif
        <div class="block w-[80%] p-6 bg-transparent border border-gray-200 mt-12 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700 overflow-y-auto">
            @foreach ($suratMasuk as $item)
                <form class="space-y-6" action="{{route('editIncomingMessage', ['id' => $item->id])}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="flex justify-between">
                        <h5 class="text-2xl font-medium text-gray-900 dark:text-white">Edit Surat Masuk</h5>
                        <a href="{{route('viewSuratMasuk')}}" class="flex items-center border justify-center px-4 h-10 text-base font-medium text-white bg-yellow-500 rounded hover:bg-yellow-600 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_surat" value="{{date('Y-m-d', strtotime($item->tanggal_surat))}}" id="tanggal_surat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>
                        <div>
                            <label for="perihal" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Perihal</label>
                            <input type="text" name="perihal" value="{{$item->perihal}}" id="perihal" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>
                        <div>
                            <label for="dari" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Dari</label>
                            <input type="text" name="dari" value="{{$item->dari}}" id="dari" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required>
                        </div>
                        <div>
                        ...
                        <div>
                            <label for="disposisi_kapolda" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Disposisi Kapolda</label>
                            <textarea name="disposisi_kapolda" id="disposisi_kapolda" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" cols="30" rows="4" required>{{$item->disposisi_kapolda}}</textarea>
                        </div>
                        <div>
                            <label for="disposisi_karo_sdm" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Disposisi Karo SDM</label>
                            <textarea name="disposisi_karo_sdm" id="disposisi_karo_sdm" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" cols="30" rows="4" required>{{$item->disposisi_karo_sdm}}</textarea>
                        </div>
                        <div>   
                    ...
                    <button type="submit" class="w-full text-white bg-green-600 border hover:bg-green-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Edit Surat Masuk</button>
                </form> 
            @endforeach
        </div>
    </div>
